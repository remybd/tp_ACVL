...
public class EditionTransition extends FenetrePopup implements ActionListener{
    ...
    public void actionPerformed(ActionEvent arg0){
        ...
        String etiquette = evenement_transition.getText().replace("[", "").replace("]", "").replace("/", "") + " [" + garde_transition.getText().replace("[", "").replace("]", "").replace("/", "") + "] / " + action_transition.getText().replace("[", "").replace("]", "").replace("/", "");
        try {
