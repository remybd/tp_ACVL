...
public class CreationTransition extends FenetrePopup implements ActionListener{
    ...
    public void actionPerformed(ActionEvent arg0){
        ...
        String etiquette = this.evenement_transition.getText().replace("[", "").replace("]", "").replace("/", "") + " [" + this.garde_transition.getText().replace("[", "").replace("]", "").replace("/", "") + "] / " + this.action_transition.getText().replace("[", "").replace("]", "").replace("/", "");
        try {
